...
DeterminingElementForThresholdCalculations <- function(DataframeOfDeconvolutedCounts){
  ...
  while(IndividualPlot == c("Y")){
    ...
    if(ElementName %in% colnames(DataframeOfDeconvolutedCounts) == TRUE){
      ...
      print(paste0(ElementName, " is not present in your data. Please select an element that is from the list: ",
                   paste(as.character(colnames(DataframeOfDeconvolutedCounts %>% select(-"Distance", -"Atom_Count"))),collapse= ", ")))
    }
