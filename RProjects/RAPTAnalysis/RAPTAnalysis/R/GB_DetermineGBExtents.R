...
DetermineGBExtents <- function(PosFile,
                               NumberOfBins,
                               Direction,
  ...
  ElementCountDF <- SplitComplexIons(DataframeOfCounts = OneDCount)

  DeterminingElementForThresholdCalculations(DataframeOfDeconvolutedCounts = ElementCountDF)

  ThresholdCheckFunction(DataframeOfDeconvolutedCounts  = ElementCountDF,
                         PValue = PValue)
  ...
  GibbsExcessCalculator(Area = Area,
                        ...
                        Grain1 = GrainA,
                        Grain2 = GrainB,
                        DataframeOfDeconvolutedCounts = ElementCountDF) #ROI Area and machine detection efficiency

  GrainBoundarySumm(Area, GrainA, GrainB, GB, CrossSectionType = CrossSectionAreaType)
